...
//    private static final Logger LOG = LoggerFactory.getLogger(CustomFontTextView.class);
    ...
    public CustomFontHelper(TextView textView) {
        mTextView = textView;
    }
     ...
     */
    public void parseAttributesAndSetFont(Context context, AttributeSet attributeSet, int defStyleAttr) {
        ...
        setFont(context, fontFamily, fontStyle);
    }
     ...
     */
    private void setFont(Context context, @Nullable String fontFamily, int fontStyle) {
        if (TextUtils.isEmpty(fontFamily)) {
            // No font family: use default font with the font style
            setSystemFont(null, fontStyle);
        } else if (!setCustomFont(context, fontFamily, fontStyle)) {
            // Custom font couldn't be set (non-existing font family, system font or error)
            // Try it as a system font
            if (!setSystemFont(fontFamily, fontStyle)) {
     ...
     */
    private boolean setCustomFont(Context context, String fontPath, int fontStyle) {
        boolean success = false;
        try {
            Typeface typeface = Typeface.createFromAsset(context.getAssets(), fontPath);
            Typeface styledTypeface = Typeface.create(typeface, fontStyle);
            mTextView.setTypeface(styledTypeface);
            success = true;
        } catch (Exception e) {
            // TODO: logging comes in a later phase
//            LOG.error("Typeface couldn't be created. Font file '{}' not usable.", fontFamily, e);
     ...
     * @param fontStyle  Typeface style constant
     */
    private boolean setSystemFont(@Nullable String fontFamily, int fontStyle) {
        boolean success = false;
        try {
