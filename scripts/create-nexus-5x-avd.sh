#!/usr/bin/env bash
#
# Creates a Nexus 5X avd with a name prefixed with the given string.
#
...
avd_name_prefix=$1
final_prefix="$avd_name_prefix"_"nexus-5x"
sys_image=$2

scripts/create-avd.sh "$final_prefix" "$sys_image" --device "Nexus 5X"